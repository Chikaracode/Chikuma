...
document.getElementById("btns").innerHTML = filters
  .map((btn) => {
    ...
    return `<button class= 'fil-p' onclick = "filterItems('${categoria}')">${categoria}</button>`;
  })
  .join("");
...
document.getElementById("btns").innerHTML = filters
  .map((btn) => {
    ...
    return `<button id="${
      idBtn || ""
    }" class="fil-p" onclick="filterItems('${categoria}')">${categoria}</button>`;
  })
  .join("");
...
const filterItems = (categoria) => {
  Array.from(document.querySelectorAll("[data-categoria]")).forEach(
    (producto) => (producto.style.display = "none")
  );
  const productosFiltrados = Array.from(
    document.querySelectorAll("[data-categoria='" + categoria + "']")
  );
...
function displayCart(a) {
  ...
  if (carrito.length == 0) {
    ...
    const precioTotal = carrito.reduce(
        (acc, value) => acc + (value.cantidad * value.precio), 0);
    document.getElementById("total").innerHTML = new Intl.NumberFormat('es-PE', { style:'currency', currency:'PEN'}).format(precioTotal); 
    document.getElementById("cartItem").innerHTML = carrito
      .map((items) => {
